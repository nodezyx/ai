    ...
    for (const update of updates) {
        if (update.update.message === null) {
            try {
                ...
                if (!store?.message) {
                    console.log('No message found in store for ID:', update.key.id);
                    continue;
                }
                ...
                // Determine the type for checking anti-delete status
                let type;
                if (isStatus) {
                    type = 'status';
                } else if (isGroup) {
                ...
                // Check if anti-delete is enabled for this type
                const antiDeleteStatus = await getAnti(type);
                console.log(`AntiDelete check: type=${type}, status=${antiDeleteStatus}, jid=${store.jid}`);
                
                if (!antiDeleteStatus) {
                    console.log('AntiDelete disabled for this type, skipping');
                    continue;
                }
                ...
                console.log('Processing deletion alert:', info);
                ...
                console.log('Deletion alert sent successfully');
            } catch (error) {
